cd C:\Users\k4321\software_CNU_team // 자신의 컴퓨터에 따라 위치가 다름 
C:\Users\k4321\software_CNU_team 를 자신의 파일 위치로 고치면됨

"실행방법" cmd에 한줄씩 
1번 git clone https://github.com/k4321527-commits/software_CNU_team
2번 cd C:\Users\k4321\software_CNU_team
3번 .\install --prefix=./install --enable_ui
4번 cd install\OpenLeetCode
5번 set GEMINI_API_KEY= (자신의 gemini api키 사용)
6번 openleetcodeui.bat
